    ...
    stages {
        ...
        stage("构建容器") {
            steps {
                ...
                sh label: "构建镜像", script: "cd courtyardMall && docker build -t courtyard_server:latest ."

                echo "==================构建前端服务器=================="
                sh label: "构建镜像", script: "cd courtyardMall_frontend && docker build -t courtyard_web:latest ."
            }
        }
        stage("启动服务") {
            steps {
